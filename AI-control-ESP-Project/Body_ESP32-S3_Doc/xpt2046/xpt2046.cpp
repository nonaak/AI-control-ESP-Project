...
boolean XPT2046::pressed(void)
{
    ...
    // Serial.println("Pressed!");  // UITGESCHAKELD: spam voorkomen
    if (_spi) {
    ...
    // if (abs(_zraw) >= Z_THRESHOLD) {
        ...
        // CUSTOM: Non-lineaire correctie voor T-HMI bij rotation 3
        // Gebaseerd op metingen: links (REC/PLAY) is bijna perfect, 
        // maar rechts (MENU/AI) heeft +30-35px offset
        if (_rotation == 3) {
            // X-correctie: progressieve correctie, meer aan rechterkant
            if (_xcoord > 120) {
                int16_t overshoot = _xcoord - 120;
                // Correctie neemt toe naarmate X groter wordt
                _xcoord -= (overshoot * 18) / 200;  
            }
            // Y-correctie: consistent ~15px te laag over hele scherm
            _ycoord = constrain(_ycoord - 15, 0, _vres_logic);
        }

...
static bool handleValueTouch(int16_t x, int16_t y) {
  int16_t lineY[] = {70, 90, 110, 130, 150, 170};  // Alle value lines (6 editors)
  
  for (int i = 0; i < 6; i++) {
    int16_t ly = lineY[i];
    if (y >= ly-4 && y <= ly+14) {
      if (i == 0) {  // ADS GSR Baseline
        if (inRect(x, y, 220, ly-4, 30, 20)) {
          sensorConfig.ads_gsrBaseline = max(0.0f, sensorConfig.ads_gsrBaseline - 100.0f);
          applySensorConfig();  // Direct toepassen
          return true;
        }
        if (inRect(x, y, 255, ly-4, 30, 20)) {
          sensorConfig.ads_gsrBaseline = min(32767.0f, sensorConfig.ads_gsrBaseline + 100.0f);
          applySensorConfig();
          return true;
        }
      }
      else if (i == 1) {  // ADS Pulse Baseline
        if (inRect(x, y, 220, ly-4, 30, 20)) {
          sensorConfig.ads_pulseBaseline = max(0, sensorConfig.ads_pulseBaseline - 100);
          applySensorConfig();
          return true;
        }
        if (inRect(x, y, 255, ly-4, 30, 20)) {
          sensorConfig.ads_pulseBaseline = min(32767, sensorConfig.ads_pulseBaseline + 100);
          applySensorConfig();
          return true;
        }
      }
      else if (i == 2) {  // ADS NTC Offset
        if (inRect(x, y, 220, ly-4, 30, 20)) {
          sensorConfig.ads_ntcOffset = max(-10.0f, sensorConfig.ads_ntcOffset - 0.1f);
          applySensorConfig();
          return true;
        }
        if (inRect(x, y, 255, ly-4, 30, 20)) {
          sensorConfig.ads_ntcOffset = min(10.0f, sensorConfig.ads_ntcOffset + 0.1f);
          applySensorConfig();
          return true;
        }
      }
      else if (i == 3) {  // Beat threshold
        if (inRect(x, y, 220, ly-4, 30, 20)) {
      ...
      else if (i == 5) {  // Temp smoothing (oude)
        if (inRect(x, y, 220, ly-4, 30, 20)) {
...
void loadSensorConfig() {
  ...
  if (loaded.magic == 0xDEADBEEF) {
    sensorConfig = loaded;
    Serial.println("[CONFIG] Kalibratie geladen uit EEPROM");
    applySensorConfig(); // Pas kalibratie toe op ADS1115
  } else {
    // First time - use defaults
    Serial.println("[CONFIG] Geen kalibratie in EEPROM, gebruik defaults");
    resetSensorConfig();
    saveSensorConfig();
  }
}
...
void applySensorConfig() {
  ...
  Serial.println("[CONFIG] ADS1115 kalibratie toegepast");
}
...
void sensorSettings_begin(Arduino_GFX* gfx) {
  ...
  // ADS1115 Kalibratie (van auto-kalibratie menu)
  drawValueLine(70, "ADS GSR Base:", sensorConfig.ads_gsrBaseline, 0.0f, 32767.0f, 100.0f);
  drawIntValueLine(90, "ADS Pulse Base:", sensorConfig.ads_pulseBaseline, 0, 32767, 100);
  drawValueLine(110, "ADS Temp Offset:", sensorConfig.ads_ntcOffset, -10.0f, 10.0f, 0.1f);
  
  // HR Settings (LED Power weggehaald - PCB LED blijft uit)
  drawValueLine(130, "Beat Threshold:", sensorConfig.hrThreshold, 10000.0f, 100000.0f, 1000.0f);
  
  // Temperature Settings (oude, voor referentie)
  drawValueLine(150, "Temp Offset:", sensorConfig.tempOffset, -10.0f, 10.0f, 0.1f);
  drawValueLine(170, "Temp Smoothing:", sensorConfig.tempSmoothing, 0.1f, 1.0f, 0.1f);
...
SensorEvent sensorSettings_poll() {
  ...
  if (inRect(x, y, bxSaveX, bxSaveY, bxSaveW, bxSaveH)) {
    ...
    Serial.println("[CONFIG] Settings opgeslagen in EEPROM");
    return SE_SAVE;
  }

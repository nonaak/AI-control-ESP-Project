...
#include "nvs_settings.h"       // 🔥 NIEUW: Centrale NVS opslag (vervangt externe EEPROM)
      ...
      if (!encoderActiveMainScreen) {
        if (pressDuration >= 1500) {
          ...
          if (waitingForDoubleClick && (millis() - lastClickTime < 800)) {
            ...
            // 🔥 NIEUW: ML sessie + recording koppelen aan AI
            if (aiOverruleActive) {
              ...
              Serial.println("[ENCODER] AI ON + Recording + ML session started");
            } else {
              ...
              Serial.println("[ENCODER] AI OFF + Recording + ML session stopped");
            }
  ...
  // ===== MENU MODE ===== (bestaande code blijft)
    ...
    if (delta != 0) {
      ...
      //else if (bodyMenuPage == BODY_PAGE_RECORDING) maxItems = 3;  // 4 buttons: 0,1,2,3
        ...
        extern bool stressPopupActive;
        extern bool stressLevelConfirmed;
        if (stressPopupActive) {
          if (!stressLevelConfirmed) {
            maxItems = 7;  // 8 stress levels (0-7)
          } else {
            maxItems = 1;  // 2 knoppen (ANNULEREN, OPSLAAN)
          }
        } else {
          maxItems = 4;  // STOP, PAUZE, AI-ACTIE, Speed-, Speed+
        }
      }
      ...
      // SPECIAL: Stress Popup scroll handling
      if (bodyMenuPage == BODY_PAGE_PLAYBACK) {
        extern bool stressPopupActive;
        extern bool stressLevelConfirmed;
        extern int selectedStressLevel;
        extern int stressPopupButtonIdx;
        
        if (stressPopupActive) {
          if (!stressLevelConfirmed) {
            // Scroll door stress levels
            if (delta > 0) {
              selectedStressLevel--;
            } else {
              selectedStressLevel++;
            }
            if (selectedStressLevel < 0) selectedStressLevel = 7;
            if (selectedStressLevel > 7) selectedStressLevel = 0;
            setEncoderLED(0, 255, 0);  // Groen (gedimd) bij level selectie
            Serial.printf("[ENCODER] Stress level: %d\n", selectedStressLevel);
          } else {
            // Scroll door knoppen
            if (delta > 0) {
              stressPopupButtonIdx--;
            } else {
              stressPopupButtonIdx++;
            }
            if (stressPopupButtonIdx < 0) stressPopupButtonIdx = 1;
            if (stressPopupButtonIdx > 1) stressPopupButtonIdx = 0;
            // LED kleur per knop
            if (stressPopupButtonIdx == 0) {
              setEncoderLED(0, 0, 255);  // Blauw voor ANNULEREN
            } else {
              setEncoderLED(0, 255, 0);  // Groen voor OPSLAAN
            }
            Serial.printf("[ENCODER] Stress button: %d\n", stressPopupButtonIdx);
          }
          bodyMenuForceRedraw();
          lastPosition = newPosition;
          return;  // Skip normale scroll
        }
      }
    ...
    if (!buttonPressed && lastButtonState) {
      ...
      //W3}
        ...
        if (bodyMenuIdx == 0) {
          // Toggle recording + ML sessie
          isRecording = !isRecording;
          if (isRecording) {
            startCSVRecording();
            mlIntegration_startLiveSession();
            Serial.println("[ENCODER] Recording + ML session START");
          } else {
            mlIntegration_stopLiveSession();
            stopCSVRecording();
            Serial.println("[ENCODER] Recording + ML session STOP");
          }
        } else if (bodyMenuIdx == 1) {
          // Model Trainen
          Serial.println("[ENCODER] Model Trainen gestart...");
          if (mlIntegration_trainModel()) {
            Serial.println("[ENCODER] ✅ Model training compleet!");
          } else {
            Serial.println("[ENCODER] ❌ Model training gefaald (te weinig data?)");
          }
        } else if (bodyMenuIdx == 2) {
          // Feedback - TODO: open playback voor annotaties
          Serial.println("[ENCODER] Feedback - TODO");
        } else if (bodyMenuIdx == 3) {
          // Model Manager - TODO
        ...
        if (!recordingInButtonMode) {
          ...
          //if (bodyMenuIdx == 0) {
            ...
            extern int csvCount;
            extern String csvFiles[];
            extern bool performAIAnalysis(const String& csvFilename);
            
            if (selectedRecordingFile >= 0 && selectedRecordingFile < csvCount) {
              String filename = csvFiles[selectedRecordingFile];
              // Check of het al een .anl bestand is
              if (filename.endsWith(".anl")) {
                Serial.println("[ENCODER] Dit bestand is al geanalyseerd (.anl)!");
              } else {
                Serial.printf("[ENCODER] Starting AI analyze: %s\n", filename.c_str());
                // Zet LED op magenta tijdens analyse
                setEncoderLED(255, 0, 255, true);
                performAIAnalysis(filename);
                selectedRecordingFile = -1;
                recordingInButtonMode = false;
                bodyMenuIdx = 0;
                csvCount = -1;  // Force rescan
                updateEncoderLEDForMenu();
              }
            } else {
              Serial.println("[ENCODER] Selecteer eerst een bestand!");
            }
          } else if (bodyMenuIdx == 3) {
        ...
        // Check eerst of stress popup actief is
        extern bool stressPopupActive;
        extern bool stressLevelConfirmed;
        extern int selectedStressLevel;
        extern int stressPopupButtonIdx;
        ...
        if (stressPopupActive) {
          if (!stressLevelConfirmed) {
            // Bevestig stress level selectie
            stressLevelConfirmed = true;
            stressPopupButtonIdx = 1;  // Default naar OPSLAAN
            setEncoderLED(255, 165, 0, true);  // Oranje voor knop selectie
            Serial.printf("[ENCODER] Stress level bevestigd: %d\n", selectedStressLevel);
          } else {
            // Voer knop actie uit
            if (stressPopupButtonIdx == 0) {

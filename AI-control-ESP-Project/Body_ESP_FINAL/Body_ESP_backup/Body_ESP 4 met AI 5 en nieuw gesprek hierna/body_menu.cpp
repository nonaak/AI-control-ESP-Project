...
void updatePlayback();   // 🔥 Forward declaration
void stopPlayback();     // 🔥 Forward declaration
void startPlayback(const char* filename);  // 🔥 Forward declaration
void drawStressLevelPopup();
bool performAIAnalysis(const String& csvFilename);
void drawAIAnalyzeScreen(int progress, const String& filename);
...
static float playbackLastHR = 0;
static float playbackLastTemp = 0;
static float playbackLastGSR = 0;
static float playbackTimestamp = 0;    // Tijd in seconden in de recording
static int playbackLastAILevel = -1;   // AI level uit .anl (indien beschikbaar)
...
bool stressPopupActive = false;
int selectedStressLevel = 2;  // Default: normaal
bool stressLevelConfirmed = false;  // true = nu bij knoppen, false = nog bij levels
int stressPopupButtonIdx = 0;  // 0=ANNULEREN, 1=OPSLAAN
...
void bodyMenuTick() {
  ...
  // 🔥 FIX: Update playback DATA (lees CSV regels!)
  if (isPlaybackActive && bodyMenuPage == BODY_PAGE_PLAYBACK && !isPlaybackPaused) {
    updatePlayback();  // Dit leest de CSV en update playbackLastHR etc.
    menuDirty = true;  // 🔥 FIX: Forceer hertekenen na elke CSV regel!
  }
  
  // Update playback DISPLAY - herteken alleen progress bar als significante verandering
...
void bodyMenuDraw() {
  ...
  // PLAYBACK mode: teken V2 scherm (body_gfx4 is UITGESCHAKELD)
  if (bodyMenuPage == BODY_PAGE_PLAYBACK) {
    g4_pauseRendering = true;  // 🔥 V2: Stop alle body_gfx4 rendering
    
    // Teken popup OVER alles ALLEEN als actief
    if (stressPopupActive) {
      drawStressLevelPopup();  // Popup OVERTEKENT alles
    } else {
      drawPlaybackScreen();  // V2 playback scherm
    }
    return;
  }
...
// ===== AI ANALYZE FUNCTIE =====
// Analyseert CSV bestand met ML model en slaat op als .ANL
bool aiAnalyzeActive = false;
int aiAnalyzeProgress = 0;
String aiAnalyzeFilename = "";
...
void drawAIAnalyzeScreen(int progress, const String& filename) {
  ...
  body_gfx->setTextColor(0xF81F, BODY_CFG.COL_BG);  // Magenta
  int16_t x1, y1; uint16_t tw, th;
  body_gfx->getTextBounds("AI ANALYSE", 0, 0, &x1, &y1, &tw, &th);
  body_gfx->setCursor((480 - tw) / 2 - x1, 50);
  body_gfx->print("AI ANALYSE");
  ...
  body_gfx->setCursor(40, 100);
  body_gfx->print("Bestand: ");
  body_gfx->setTextColor(0x07E0, BODY_CFG.COL_BG);  // Groen
  ...
  // Voortgangsbalk achtergrond
  int barX = 40;
  int barY = 150;
  int barW = 400;
  int barH = 40;
  
  body_gfx->drawRoundRect(barX, barY, barW, barH, 8, 0xFFFF);
  body_gfx->drawRoundRect(barX+1, barY+1, barW-2, barH-2, 7, 0xFFFF);
  
  // Voortgangsbalk vulling (roze/magenta gradient)
  int fillW = (barW - 8) * progress / 100;
  if (fillW > 0) {
    body_gfx->fillRoundRect(barX + 4, barY + 4, fillW, barH - 8, 4, 0xF81F);
  }
  ...
  body_gfx->setTextColor(0xFFFF, BODY_CFG.COL_BG);
  char percentStr[8];
  sprintf(percentStr, "%d%%", progress);
  body_gfx->getTextBounds(percentStr, 0, 0, &x1, &y1, &tw, &th);
  body_gfx->setCursor((480 - tw) / 2 - x1, 220);
  body_gfx->print(percentStr);
  ...
  #endif
  body_gfx->setTextColor(0xF81F, BODY_CFG.COL_BG);  // Magenta
  body_gfx->setCursor(40, 260);
  if (progress < 100) {
    body_gfx->print("Analyseren met ML model...");
  } else {
    body_gfx->setTextColor(0x07E0, BODY_CFG.COL_BG);  // Groen
    body_gfx->print("Analyse compleet! Opgeslagen als .ANL");
  }
}

bool performAIAnalysis(const String& csvFilename) {
  Serial.printf("[AI ANALYZE] Starting analysis of: %s\n", csvFilename.c_str());
  
  aiAnalyzeActive = true;
  aiAnalyzeFilename = csvFilename;
  aiAnalyzeProgress = 0;
  
  // Open CSV bestand
  String csvPath = "/recordings/" + csvFilename;
  File csvFile = SD_MMC.open(csvPath.c_str(), FILE_READ);
  if (!csvFile) {
    Serial.println("[AI ANALYZE] ERROR: Cannot open CSV file!");
    aiAnalyzeActive = false;
    return false;
  }
  
  // Tel regels voor progress
  int totalLines = 0;
  while (csvFile.available()) {
    csvFile.readStringUntil('\n');
    totalLines++;
  }
  totalLines--;  // Min header
  csvFile.seek(0);
  
  Serial.printf("[AI ANALYZE] Total data lines: %d\n", totalLines);
  
  // Maak ANL bestandsnaam
  String anlFilename = csvFilename;
  anlFilename.replace(".csv", ".anl");
  String anlPath = "/recordings/" + anlFilename;
  
  File anlFile = SD_MMC.open(anlPath.c_str(), FILE_WRITE);
  if (!anlFile) {
    Serial.println("[AI ANALYZE] ERROR: Cannot create ANL file!");
    csvFile.close();
    aiAnalyzeActive = false;
    return false;
  }
  
  // Lees en schrijf header + StressLevel kolom
  String header = csvFile.readStringUntil('\n');
  header.trim();
  anlFile.println(header + ",StressLevel");
  
  // Teken eerste scherm
  drawAIAnalyzeScreen(0, csvFilename);
  
  // ═══════════════════════════════════════════════════════════════════
  // AI BIJBEL STRESS LEVEL CLASSIFIER
  // 
  ...
  // AI Bijbel baseline waarden (BIJBEL_ prefix om conflict met config.h te voorkomen)
  const float BIJBEL_HR_BASELINE = 70.0f;    // Gemiddelde hartslag in rust
  const float BIJBEL_HR_EXCITED = 100.0f;    // Hartslag bij opwinding
  const float BIJBEL_HR_EDGE = 130.0f;       // Hartslag bij edge zone
  const float BIJBEL_HR_MAX = 160.0f;        // Maximum veilige hartslag
  
  const float BIJBEL_TEMP_BASELINE = 36.5f;  // Normale temperatuur
  const float BIJBEL_TEMP_ELEVATED = 37.5f;  // Verhoogde temperatuur
  const float BIJBEL_TEMP_HIGH = 38.0f;      // Hoge temperatuur
  
  const float BIJBEL_GSR_BASELINE = 300.0f;  // GSR in rust
  const float BIJBEL_GSR_AROUSED = 600.0f;   // GSR bij opwinding
  const float BIJBEL_GSR_EDGE = 900.0f;      // GSR bij edge zone
  const float BIJBEL_GSR_MAX = 1200.0f;      // Maximum GSR
  ...
  while (csvFile.available()) {
    ...
    // Parse CSV: Tijd_s,Timestamp,BPM,Temp_C,GSR,...
    int commaIdx[15];
    int commaCount = 0;
    ...
    int stressLevel = 3;  // Default: normaal
    
    if (commaCount >= 4) {
      // Extract BPM, Temp, GSR (kolommen 2, 3, 4 = index 1, 2, 3)
      float bpm = line.substring(commaIdx[1] + 1, commaIdx[2]).toFloat();
      float temp = line.substring(commaIdx[2] + 1, commaIdx[3]).toFloat();
      float gsr = line.substring(commaIdx[3] + 1, commaIdx[4]).toFloat();
      
      // ═══════════════════════════════════════════════════════════════
      // MULTI-FACTOR STRESS BEREKENING (AI BIJBEL GEBASEERD)
      // ═══════════════════════════════════════════════════════════════
      
      // Hartslag score (0.0 - 1.0)
      float hrScore = 0.0f;
      if (bpm > 0) {
        if (bpm <= BIJBEL_HR_BASELINE) {
          hrScore = 0.0f;  // Onder baseline = ontspannen
        } else if (bpm <= BIJBEL_HR_EXCITED) {
          hrScore = (bpm - BIJBEL_HR_BASELINE) / (BIJBEL_HR_EXCITED - BIJBEL_HR_BASELINE) * 0.4f;
        } else if (bpm <= BIJBEL_HR_EDGE) {
          hrScore = 0.4f + (bpm - BIJBEL_HR_EXCITED) / (BIJBEL_HR_EDGE - BIJBEL_HR_EXCITED) * 0.4f;
        } else {
          hrScore = 0.8f + (bpm - BIJBEL_HR_EDGE) / (BIJBEL_HR_MAX - BIJBEL_HR_EDGE) * 0.2f;
        }
      }
      ...
      if (temp > 0) {
        if (temp <= BIJBEL_TEMP_BASELINE) {
          tempScore = 0.0f;
        } else if (temp <= BIJBEL_TEMP_ELEVATED) {
          tempScore = (temp - BIJBEL_TEMP_BASELINE) / (BIJBEL_TEMP_ELEVATED - BIJBEL_TEMP_BASELINE) * 0.5f;
        } else {
          tempScore = 0.5f + (temp - BIJBEL_TEMP_ELEVATED) / (BIJBEL_TEMP_HIGH - BIJBEL_TEMP_ELEVATED) * 0.5f;
        }
      }
      ...
      // Gewogen combinatie
      // HR weegt het zwaarst (50%), dan GSR (35%), dan temp (15%)
      float combinedScore = (hrScore * 0.50f) + (gsrScore * 0.35f) + (tempScore * 0.15f);
      combinedScore = constrain(combinedScore, 0.0f, 1.0f);
      
      // Converteer naar 0-7 level (AI Bijbel)
      ...
      // Special case: Zeer hoge hartslag = altijd hoog stress
      if (bpm > BIJBEL_HR_EDGE) {
        stressLevel = max(stressLevel, 6);
      }
      if (bpm > BIJBEL_HR_MAX) {
        stressLevel = 7;  // Emergency zone
      }
    }
    
    // Schrijf regel + stress level
    anlFile.printf("%s,%d\n", line.c_str(), stressLevel);
    
    // Update progress (elke 5% of elke 50 regels)
    int progress = (lineNum * 100) / max(1, totalLines);
    if (progress != lastProgress && (progress % 5 == 0 || lineNum % 50 == 0)) {
      lastProgress = progress;
      aiAnalyzeProgress = progress;
      drawAIAnalyzeScreen(progress, csvFilename);
  ...
  // Toon 100% compleet
  aiAnalyzeProgress = 100;
  drawAIAnalyzeScreen(100, csvFilename);
  
  Serial.printf("[AI ANALYZE] Complete! Saved as: %s\n", anlFilename.c_str());
  
  // Wacht even zodat gebruiker resultaat ziet
  delay(1500);
  
  // Reset state
  aiAnalyzeActive = false;
  csvCount = -1;  // Force rescan om nieuwe .anl te tonen
...
void drawRecordingItems() {
  ...
  // Scan SD card voor .csv EN .anl bestanden in /recordings/ (gebruikt globale variabelen)
    ...
    if (root && root.isDirectory()) {
      ...
      while (file && csvCount < 50) {
        if (!file.isDirectory()) {
          ...
          if (filename.endsWith(".csv") || filename.endsWith(".anl")) {
  ...
  if (csvCount == 0) {
    ...
    for (int i = 0; i < 8 && (i + scrollOffset) < csvCount; i++) {
      ...
      // ROZE/MAGENTA = .anl bestand (al geanalyseerd)
        ...
        body_gfx->setTextColor(0xF81F, BODY_CFG.COL_BG);  // Magenta/roze letters
      }
      // Normaal = .csv bestand
      else {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if (x >= BTN_X && x <= BTN_X + BTN_W && y >= btnY && y <= btnY + BTN_H) {
            ...
            switch(i) {
              ...
              case 2:
                if (selectedRecordingFile >= 0 && selectedRecordingFile < csvCount) {
                  String filename = csvFiles[selectedRecordingFile];
                  // Check of het al een .anl bestand is
                  if (filename.endsWith(".anl")) {
                    Serial.println("[RECORDING] Dit bestand is al geanalyseerd (.anl)!");
                  } else {
                    Serial.printf("[RECORDING] Starting AI analyze: %s\n", filename.c_str());
      ...
      if (bodyMenuPage == BODY_PAGE_PLAYBACK) {
        ...
        if (stressPopupActive) {
          ...
          if (x >= btn2X && x <= btn2X + btnW && y >= btnY && y <= btnY + btnH) {
            ...
            // Voeg marker toe (visuele weergave)
            ...
            // 🔥 NIEUW: Sla op voor ML training!
            // Dit wordt later gebruikt om ML te trainen
            extern bool ml_addPlaybackAnnotation(float, int, float, float, float, int);
            if (ml_addPlaybackAnnotation(playbackTimestamp, selectedStressLevel,
                                          playbackLastHR, playbackLastTemp, playbackLastGSR,
                                          playbackLastAILevel)) {
              Serial.println("[PLAYBACK] ✅ Annotatie opgeslagen voor ML training!");
              Serial.printf("[PLAYBACK]    Timestamp: %.1fs, Level: %d\n", playbackTimestamp, selectedStressLevel);
              Serial.printf("[PLAYBACK]    HR=%.0f, Temp=%.1f, GSR=%.0f\n", 
                           playbackLastHR, playbackLastTemp, playbackLastGSR);
              if (playbackLastAILevel >= 0) {
                Serial.printf("[PLAYBACK]    AI had voorspeld: %d (correctie: %+d)\n",
                             playbackLastAILevel, selectedStressLevel - playbackLastAILevel);
              }
            }
        ...
        if (x >= btn4X && x <= btn4X + btnW && y >= btnY && y <= btnY + btnH) {
          ...
          extern int ml_getAnnotationAt(float);
          
          static bool annotationFileOpened = false;
          if (!annotationFileOpened && selectedPlaybackFile[0] != '\0') {
            ml_openAnnotationFile(String(selectedPlaybackFile));
            annotationFileOpened = true;
          }
          ...
          if (existingLevel >= 0) {
            ...
            Serial.printf("[PLAYBACK] Bestaande annotatie gevonden: level %d\n", existingLevel);
          } else if (playbackLastAILevel >= 0) {
            // Gebruik AI voorspelling als start waarde
            selectedStressLevel = playbackLastAILevel;
            Serial.printf("[PLAYBACK] Geen annotatie, AI level: %d\n", playbackLastAILevel);
          } else {
            // Default
            selectedStressLevel = 3;  // Licht verhoogd als default
          }
          
          stressPopupActive = true;
          stressLevelConfirmed = false;  // Start bij level selectie
          stressPopupButtonIdx = 1;      // Default naar OPSLAAN
          
          // Pauzeer playback tijdens popup
...
void drawPlaybackScreen() {
  ...
  // 🔥 DEBUG: Print waarden elke 50 frames
  static int drawDebugCounter = 0;
  if (++drawDebugCounter % 50 == 0) {
    Serial.printf("[DRAW] Playback values: HR=%.1f, T=%.1f, GSR=%.1f, File='%s'\n",
                  playbackLastHR, playbackLastTemp, playbackLastGSR, playbackFilename.c_str());
  }
  ...
  // Gebruik PLAYBACK CSV data, niet live sensors!
  playbackScreen.setSensorValues(playbackLastHR, playbackLastTemp, playbackLastGSR);
  
  // Update levels
  int displayLevel = (playbackLastAILevel >= 0) ? playbackLastAILevel : selectedStressLevel;
  playbackScreen.setCurrentLevel(displayLevel);
  playbackScreen.setAIPrediction(playbackLastAILevel);  // AI level uit .anl
  // User annotation wordt gezet na GEVOEL popup
...
void drawStressLevelPopup() {
  ...
  // 8 stress levels (0-7) als radio buttons
  #if USE_ADAFRUIT_FONTS
    body_gfx->setFont(&FONT_ITEM);
    body_gfx->setTextSize(1);
  #else
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (i == selectedStressLevel) {
      body_gfx->fillCircle(circleX, circleY, circleR - 2, stressLevelConfirmed ? 0xF800 : 0x07E0);  // Rood als bevestigd, groen als selecterend
    }
    
    // Label - kleur op basis van encoder selectie
    uint16_t labelColor = 0xFFFF;  // Default wit
    if (i == selectedStressLevel && !stressLevelConfirmed) {
      labelColor = 0x07E0;  // Groen als aan het selecteren
    } else if (i == selectedStressLevel && stressLevelConfirmed) {
      labelColor = 0xF800;  // Rood als bevestigd
    }
    body_gfx->setTextColor(labelColor, 0x2104);
  ...
  if (stressLevelConfirmed && stressPopupButtonIdx == 0) {
    // Encoder highlight: oranje rand
    body_gfx->drawRoundRect(btn1X-2, btnY-2, btnW+4, btnH+4, 10, 0xFD20);
    body_gfx->drawRoundRect(btn1X-1, btnY-1, btnW+2, btnH+2, 9, 0xFD20);
  }
  body_gfx->drawRoundRect(btn1X, btnY, btnW, btnH, 8, 0xFFFF);
  ...
  if (stressLevelConfirmed && stressPopupButtonIdx == 1) {
    // Encoder highlight: oranje rand
    body_gfx->drawRoundRect(btn2X-2, btnY-2, btnW+4, btnH+4, 10, 0xFD20);
    body_gfx->drawRoundRect(btn2X-1, btnY-1, btnW+2, btnH+2, 9, 0xFD20);
  }
...
void startPlayback(const char* filename) {
  ...
  // Tel aantal regels (voor progress) - MET watchdog feeding!
  ...
  while (playbackFile.available()) {
    ...
    if (line.length() > 0) {
      ...
      // 🔥 FIX: Feed watchdog elke 100 regels om crash te voorkomen
      if (lineCounter >= 100) {
        esp_task_wdt_reset();
        lineCounter = 0;
      }
  ...
  Serial.printf("[PLAYBACK] V2 filename: '%s', duration: %.0f sec\n", 
                playbackFilename.c_str(), playbackDuration);
  
  // Reset naar begin en skip header
  playbackFile.seek(0);
  if (playbackFile.available()) {
    String header = playbackFile.readStringUntil('\n');  // Skip header
    Serial.printf("[PLAYBACK] CSV header: %s\n", header.substring(0, 80).c_str());  // Eerste 80 chars
  }
  ...
  // 🔥 V2: Reset en initialiseer playback screen
  g4_pauseRendering = true;  // Stop body_gfx4 grafieken
...
void updatePlayback() {
  ...
  // 🔥 DEBUG: Print eerste paar regels RAW
  if (playbackCurrentLine <= 3) {
    Serial.printf("[PLAYBACK] === LINE %d RAW ===\n", playbackCurrentLine);
    Serial.printf("  Full line: %s\n", line.c_str());
  }
  ...
  String timeStr = line.substring(0, commaPos[0]);                    // Kolom 0: Tijd_s
  ...
  float trust = trustStr.toFloat();
  float sleeve = sleeveStr.toFloat();
  int vibe = vibeStr.toInt();
  int zuig = zuigStr.toInt();
  
  // 🔥 DEBUG: Print parsed waarden voor eerste regels en elke 10 regels
  if (playbackCurrentLine <= 3) {
    Serial.printf("[PLAYBACK] === PARSED LINE %d ===\n", playbackCurrentLine);
    Serial.printf("  hrStr='%s' -> HR=%.1f\n", hrStr.c_str(), hr);
    Serial.printf("  tempStr='%s' -> Temp=%.1f\n", tempStr.c_str(), temp);
    Serial.printf("  gsrStr='%s' -> GSR=%.1f\n", gsrStr.c_str(), gsr);
  }
  
  static int debugCounter = 0;
  if (++debugCounter % 50 == 0) {
    Serial.printf("[PLAYBACK] Line %d: HR=%.1f, T=%.1f, GSR=%.1f\n", 
                  playbackCurrentLine, hr, temp, gsr);
  }
  
  // 🔥 NIEUW: Sla sensor waarden op voor ML annotatie
  playbackTimestamp = timeStr.toFloat();
  playbackLastHR = hr;
  playbackLastTemp = temp;
  playbackLastGSR = gsr;
  
  // Check of er een StressLevel kolom is (voor .anl bestanden)
  // Format: ...,StressLevel als laatste kolom
  if (commaCount >= 14) {
    // Probeer laatste kolom te parsen als AI level
    String lastCol = line.substring(commaPos[commaCount-1] + 1);
    playbackLastAILevel = lastCol.toInt();
  } else {
    playbackLastAILevel = -1;  // Geen AI level beschikbaar
  }
  
  // 🔥 V2: Grafieken worden nu in playback_screen_v2 getekend
  // De oude body_gfx4 grafieken zijn uitgeschakeld voor cleaner v2 layout
  // body_gfx4_pushSample(G4_HART, hr, false);
  // body_gfx4_pushSample(G4_HUID, gsr / 10.0f, false);
  // body_gfx4_pushSample(G4_TEMP, temp, false);
  // body_gfx4_pushSample(G4_ADEMHALING, 0, false);
  // body_gfx4_pushSample(G4_HOOFDESP, trust, false);
  // body_gfx4_pushSample(G4_ZUIGEN, zuig, false);
  // body_gfx4_pushSample(G4_TRIL, vibe, false);
  
  // Update v2 screen met sensor data
  playbackScreen.setSensorValues(hr, temp, gsr);
  playbackScreen.pushLevelSample(playbackLastAILevel >= 0 ? playbackLastAILevel : 3);
...
void stopPlayback() {
  ...
  // 🔥 NIEUW: Sluit en sla annotaties op voor ML training
  extern void ml_closeAnnotationFile();
  extern bool ml_exportAnnotationsForTraining();
  
  ml_closeAnnotationFile();
  Serial.println("[PLAYBACK] Annotaties opgeslagen voor ML training");
  
  // Export naar ML training formaat
  if (ml_exportAnnotationsForTraining()) {
    Serial.println("[PLAYBACK] ✅ Annotaties geëxporteerd naar /ml_training/");
    Serial.println("[PLAYBACK] Gebruik ml_trainModel() om het model te updaten!");
  }
  
  // Sla stress markers op naar .preview bestand (visuele weergave)
  if (stressMarkerCount > 0) {
    saveStressMarkers();
  }
  
  // Reset annotatie tracking
  static bool annotationFileOpened = false;
  annotationFileOpened = false;

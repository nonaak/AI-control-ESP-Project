/*  
  Body ESP T-HMI – 4-band + Menu + Touch-kalibratie (geporteerd voor T-HMI ESP32-S3)
  - DISPLAY_ROTATION = 2 (matcht jouw werkende code patroon)
  - Gebruikt TFT_eSPI library voor T-HMI parallel display
*/

// T-HMI pin configuratie (uit werkende T-HMI example)
...
// SD card pins (T-HMI specificaties)
...
// T-HMI Display definitie (TFT_eSPI)
TFT_eSPI tft = TFT_eSPI(); // T-HMI display object
// Extern reference voor compatibiliteit met body_display system
TFT_eSPI *body_gfx_tft = &tft;

// ===== SD Card ===== (T-HMI gebruikt SD_MMC)
// SD_CS_PIN niet nodig voor SD_MMC
...
static uint8_t bodyESP_MAC[] = {0xEC, 0xDA, 0x3B, 0x98, 0xC5, 0x24};   // Body ESP (deze T-HMI unit)
...
// SD card functies (T-HMI SD interface)
static void initSDCard() {
  // T-HMI SD card initialisatie met SD_MMC
  ...
  if (SD_MMC.begin("/sdcard", true)) {
    ...
    Serial.printf("[T-HMI] SD card gereed - %.2f GB\n", SD_MMC.cardSize() / 1024.0 / 1024.0 / 1024.0);
  } else {
    sdCardAvailable = false;
    Serial.println("[T-HMI] SD card fout!");
  }
  delay(500);
}
...
static void initEventLogging() {
  ...
  eventLogFile = SD_MMC.open(filename, FILE_WRITE);
...
static void startPlayback(String filename) {
  ...
  playbackFile = SD_MMC.open("/" + filename);
  if (playbackFile) {
...
void setup() {
  // T-HMI Power management (KRITIEK!)
  pinMode(PWR_EN_PIN, OUTPUT);
  digitalWrite(PWR_EN_PIN, HIGH);
  
  Serial.begin(115200);
  delay(2000);  // Wacht om serial verbinding stabiel te maken
  Serial.println("[T-HMI] Starting Body ESP T-HMI...");
  Serial.println("[T-HMI] Power management init OK");
  
  Serial.println("[T-HMI] Initializing RGB off...");
  rgbOffInit();
  Serial.println("[T-HMI] RGB init OK");

  // Initialize T-HMI display system
  Serial.println("[T-HMI] Initializing TFT display...");
  tft.begin();
  tft.setRotation(DISPLAY_ROTATION);  // 1 = landscape zoals origineel
  tft.setSwapBytes(true);
  tft.fillScreen(BODY_CFG.COL_BG);
  Serial.println("[T-HMI] TFT init OK");
  
  // T-HMI brightness control (uit T-HMI example)
  pinMode(BK_LIGHT_PIN, OUTPUT);
  digitalWrite(BK_LIGHT_PIN, HIGH);
  
  Serial.println("[T-HMI] Display geïnitialiseerd");
  
  // Ensure body_gfx_tft is correctly set
  body_gfx_tft = &tft;
  ...
  Serial.println("[T-HMI] Touch cooldown ingesteld");
  
  Serial.println("[T-HMI] Initializing I2C...");
  Wire.begin(PIN_SDA, PIN_SCL);
  Wire.setClock(400000);
  Serial.println("[T-HMI] I2C OK, initializing MAX30102...");
  // Try multiple addresses to avoid EEPROM conflict (0x57)
  bool ok = max30.begin(Wire, I2C_SPEED_STANDARD) ||           // Default address
            max30.begin(Wire, I2C_SPEED_STANDARD, 0x56) ||    // Alternative address
            max30.begin(Wire, I2C_SPEED_STANDARD, 0x58) ||    // Alternative address  
            max30.begin(Wire, I2C_SPEED_STANDARD, MAX_I2C_ADDR); // Original 0x57 (may conflict with EEPROM)
  Serial.printf("[T-HMI] MAX30102 init result: %s\n", ok ? "OK" : "FAILED");
  if (!ok) {
    body_gfx->setCursor(20, 20);
    body_gfx->setTextColor(0xF800, 0x0000); // Red on black
    body_gfx->print("MAX30102 niet gevonden!");
    Serial.println("[T-HMI] MAX30102 niet gevonden - doorgaan zonder sensor");
  } else {
    Serial.println("[T-HMI] MAX30102 gevonden en geïnitialiseerd");
  }
  max30.setup();
...
void loop() {
    ...
    if (inputTouchPoll(ev)) {
      if (body_gfx4_canTouch(true)) {  // Main screen cooldown
        ...
        if (ev.kind == TE_TAP_PLAY) {
          Serial.println("[BODY] PLAY knop ingedrukt!");
          if (isPlaying) {
            stopPlayback();
            isPlaying = false;
            ...
            Serial.println("[BODY] Opening playlist...");
            // Pauze om dubbele touch te voorkomen
            delay(500);
            enterPlaylist();
            return;
          }
        ...
        if (ev.kind == TE_TAP_MENU) {
          Serial.println("[BODY] MENU knop ingedrukt!");
          uiMenu = true;
  ...
  if (mode == MODE_SYSTEM_SETTINGS) {
    ...
    if (sse == SSE_BACK) { delay(500); enterMenu(); return; }
  ...
  if (mode == MODE_SENSOR_SETTINGS) {
    ...
    if (sev == SE_BACK) { delay(500); enterMenu(); return; }
    if (sev == SE_SAVE) {
      delay(500);
      saveSensorConfig();
  ...
  if (mode == MODE_OVERRULE) {
    ...
    if (oev == OE_BACK) { delay(500); enterMenu(); return; }
    if (oev == OE_TOGGLE_AI) {
      delay(500);
  ...
  if (mode == MODE_AI_ANALYZE) {
    ...
    if (aev == AE_BACK) { delay(500); enterOverrule(); return; }
    if (aev == AE_TRAIN) {
      delay(500);
  ...
  if (mode == MODE_AI_EVENT_CONFIG) {
    ...
    if (ece == ECE_BACK) { delay(500); enterOverrule(); return; }
    if (ece == ECE_SAVE) {
      delay(500);

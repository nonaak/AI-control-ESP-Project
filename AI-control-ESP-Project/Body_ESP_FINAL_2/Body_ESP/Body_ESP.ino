...
void updateCSVRecording() {
  ...
  if (rtcAvailable) {
    ...
    // Schrijf CSV regel
    csvFile.printf("%.1f,%s,%u,%.2f,%.1f,%.2f,%.2f,%.1f,%d,%d,%.1f,%d,%.0f,%u,%d\n",
    ...
    csvFile.flush();  // Schrijf direct naar SD card
    csvSampleCount++;
    
    if (csvSampleCount % 10 == 0) {  // Elke 10 samples een status print
      Serial.printf("[CSV] Sample %u: BPM=%u Temp=%.1f GSR=%.0f\n",
                    csvSampleCount, sensorData.BPM, sensorData.temperature, sensorData.gsrSmooth);
    }
  }
}

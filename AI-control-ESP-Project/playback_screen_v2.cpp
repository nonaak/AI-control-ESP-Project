...
PlaybackScreenV2::PlaybackScreenV2() {
  ...
  sensorHistoryIdx = 0;
  sensorHistoryFull = false;
  memset(hrHistory, 0, sizeof(hrHistory));
  memset(tempHistory, 0, sizeof(tempHistory));
  memset(gsrHistory, 0, sizeof(gsrHistory));
  memset(ademHistory, 0, sizeof(ademHistory));
...
void PlaybackScreenV2::setSensorValues(float heartRate, float temperature, float gsrVal, float ademVal) {
  hr = heartRate;
  temp = temperature;
  gsr = gsrVal;
  adem = ademVal;
...
void PlaybackScreenV2::pushSensorSample(float hrVal, float tempVal, float gsrVal, float ademVal) {
  hrHistory[sensorHistoryIdx] = hrVal;
  tempHistory[sensorHistoryIdx] = tempVal;
  gsrHistory[sensorHistoryIdx] = gsrVal;
  ademHistory[sensorHistoryIdx] = ademVal;
  sensorHistoryIdx = (sensorHistoryIdx + 1) % 100;
  if (sensorHistoryIdx == 0) sensorHistoryFull = true;
}

void PlaybackScreenV2::clearSensorHistory() {
  sensorHistoryIdx = 0;
  sensorHistoryFull = false;
  memset(hrHistory, 0, sizeof(hrHistory));
  memset(tempHistory, 0, sizeof(tempHistory));
...
void PlaybackScreenV2::drawTitleBar() {
  ...
  gfx->setFont(&FreeSansOblique9pt7b);
  gfx->setCursor(30, 19);  // Aangepast
  ...
  gfx->setCursor(PB_SCREEN_W - tw - 5, 19);  // Aangepast
  gfx->print(timeStr);
  gfx->setFont(NULL);  // Terug naar standaard voor andere elementen
}
...
void PlaybackScreenV2::drawLevelPanel() {
  ...
  gfx->setFont(&FreeSansBold24pt7b);  // Mooie smooth font
  
  uint16_t textColor = (currentLevel < 3) ? 0x0000 : 0xFFFF;
  gfx->setTextColor(textColor, levelColor);
  
  // Centreer: FreeFont baseline, dus Y hoger
  int textX = boxX + (boxW / 2) - 12;
  int textY = boxY + (boxH / 2) + 15;  // 3px omlaag
  ...
  if (aiPredictedLevel >= 0) {
    ...
    gfx->setCursor(panelX + 5, compareY + 12);  // 8px omhoog
    ...
    if (userAnnotatedLevel >= 0) {
      ...
      gfx->setCursor(panelX + 5, compareY + 34);  // 8px omhoog
  ...
  gfx->setFont(NULL);  // Reset font na level panel
}
...
void PlaybackScreenV2::drawProgressBar() {
  ...
  gfx->setFont(&FreeSansBoldOblique9pt7b);
  gfx->setTextColor(0xFFFF, 0x001F);
  gfx->setCursor(speedX + 10, speedY + 18);  // 2px naar rechts
  gfx->print("-");
  ...
  gfx->setCursor(textX + (textW - tw) / 2, speedY + 21);  // Aangepast voor FreeFont
  gfx->print(speedStr);
  ...
  gfx->setCursor(plusX + 8, speedY + 18);  // 2px naar rechts
  gfx->print("+");
...
void PlaybackScreenV2::drawSensorGraphs() {
  ...
  int labelW = 55;  // Meer ruimte voor label links
  int valueW = 60;  // Meer ruimte voor waarde rechts
  int barX = graphX + labelW;
  int barW = graphW - labelW - valueW;
  int rowH = 28;
  int spacing = 3;
  int16_t x1, y1;
  uint16_t tw, th;
  
  int count = sensorHistoryFull ? 100 : sensorHistoryIdx;
  int startIdx = sensorHistoryFull ? sensorHistoryIdx : 0;
  float step = (float)barW / 100.0f;
  
  // ─── HR Grafiek ───
  int hrY = PB_GRAPH_Y + 8;
  gfx->fillRect(barX, hrY, barW, rowH - 2, 0x1082);
  gfx->setTextColor(0xF800, 0x0000);
  gfx->setCursor(graphX, hrY + 18);
  gfx->print("HR");
  // Teken lijn
  if (count > 1) {
    float hrMin = 40, hrMax = 180;  // Typisch HR bereik
    for (int i = 1; i < count; i++) {
      int idx1 = (startIdx + i - 1) % 100;
      int idx2 = (startIdx + i) % 100;
      int lx1 = barX + (int)((i - 1) * step);
      int lx2 = barX + (int)(i * step);
      int ly1 = hrY + rowH - 4 - (int)(((hrHistory[idx1] - hrMin) / (hrMax - hrMin)) * (rowH - 6));
      int ly2 = hrY + rowH - 4 - (int)(((hrHistory[idx2] - hrMin) / (hrMax - hrMin)) * (rowH - 6));
      ly1 = constrain(ly1, hrY + 2, hrY + rowH - 4);
      ly2 = constrain(ly2, hrY + 2, hrY + rowH - 4);
      gfx->drawLine(lx1, ly1, lx2, ly2, 0xF800);
    }
  }
  // Waarde rechts
  char hrStr[12];
  sprintf(hrStr, "%.0f", hr);
  gfx->setTextColor(0xF800, 0x0000);
  gfx->setCursor(barX + barW + 5, hrY + 18);
  gfx->print(hrStr);
  ...
  gfx->setCursor(graphX, tempY + 18);
  gfx->print("TEMP");
  if (count > 1) {
    float tempMin = 30, tempMax = 40;
    for (int i = 1; i < count; i++) {
      int idx1 = (startIdx + i - 1) % 100;
      int idx2 = (startIdx + i) % 100;
      int lx1 = barX + (int)((i - 1) * step);
      int lx2 = barX + (int)(i * step);
      int ly1 = tempY + rowH - 4 - (int)(((tempHistory[idx1] - tempMin) / (tempMax - tempMin)) * (rowH - 6));
      int ly2 = tempY + rowH - 4 - (int)(((tempHistory[idx2] - tempMin) / (tempMax - tempMin)) * (rowH - 6));
      ly1 = constrain(ly1, tempY + 2, tempY + rowH - 4);
      ly2 = constrain(ly2, tempY + 2, tempY + rowH - 4);
      gfx->drawLine(lx1, ly1, lx2, ly2, 0xFD20);
    }
  }
  char tempStr[12];
  sprintf(tempStr, "%.1f", temp);
  gfx->setTextColor(0xFD20, 0x0000);
  gfx->setCursor(barX + barW + 5, tempY + 18);
  gfx->print(tempStr);
  ...
  gfx->setCursor(graphX, gsrY + 18);
  gfx->print("GSR");
  if (count > 1) {
    float gsrMin = 0, gsrMax = 1000;  // GSR bereik 0-1000
    for (int i = 1; i < count; i++) {
      int idx1 = (startIdx + i - 1) % 100;
      int idx2 = (startIdx + i) % 100;
      int lx1 = barX + (int)((i - 1) * step);
      int lx2 = barX + (int)(i * step);
      int ly1 = gsrY + rowH - 4 - (int)(((gsrHistory[idx1] - gsrMin) / (gsrMax - gsrMin)) * (rowH - 6));
      int ly2 = gsrY + rowH - 4 - (int)(((gsrHistory[idx2] - gsrMin) / (gsrMax - gsrMin)) * (rowH - 6));
      ly1 = constrain(ly1, gsrY + 2, gsrY + rowH - 4);
      ly2 = constrain(ly2, gsrY + 2, gsrY + rowH - 4);
      gfx->drawLine(lx1, ly1, lx2, ly2, 0xFFE0);
    }
  }
  char gsrStr[12];
  sprintf(gsrStr, "%.0f", gsr);
  gfx->setTextColor(0xFFE0, 0x0000);
  gfx->setCursor(barX + barW + 5, gsrY + 18);
  gfx->print(gsrStr);
  
  // ─── ADEM Grafiek ───
  int ademY = gsrY + rowH + spacing;
  gfx->fillRect(barX, ademY, barW, rowH - 2, 0x1082);
  gfx->setTextColor(0x07FF, 0x0000);
  gfx->setCursor(graphX, ademY + 18);
  gfx->print("ADEM");
  if (count > 1) {
    float ademMin = 8, ademMax = 30;  // Typisch ademhaling bereik
    for (int i = 1; i < count; i++) {
      int idx1 = (startIdx + i - 1) % 100;
      int idx2 = (startIdx + i) % 100;
      int lx1 = barX + (int)((i - 1) * step);
      int lx2 = barX + (int)(i * step);
      int ly1 = ademY + rowH - 4 - (int)(((ademHistory[idx1] - ademMin) / (ademMax - ademMin)) * (rowH - 6));
      int ly2 = ademY + rowH - 4 - (int)(((ademHistory[idx2] - ademMin) / (ademMax - ademMin)) * (rowH - 6));
      ly1 = constrain(ly1, ademY + 2, ademY + rowH - 4);
      ly2 = constrain(ly2, ademY + 2, ademY + rowH - 4);
      gfx->drawLine(lx1, ly1, lx2, ly2, 0x07FF);
    }
  }
  char ademStr[12];
  sprintf(ademStr, "%.0f", adem);
  gfx->setTextColor(0x07FF, 0x0000);
  gfx->setCursor(barX + barW + 5, ademY + 18);
  gfx->print(ademStr);
...
void PlaybackScreenV2::drawLevelGraph() {
  ...
  int graphX = PB_GRAPH_X;
  int graphW = PB_GRAPH_W - 10;
  int labelW = 55;  // Meer ruimte voor LEVEL tekst
  int barX = graphX + labelW;
  int barW = graphW - labelW;  // Doorlopen tot rechts (geen valueW)
  int graphY = PB_GRAPH_Y + 142;  // Onder de sensor grafieken
  int graphH = 35;
  ...
  gfx->setCursor(graphX, graphY + (graphH / 2) + 5);  // Aangepast voor FreeFont baseline
  gfx->print("LEVEL");
  gfx->setFont(NULL);  // Reset font
...
void PlaybackScreenV2::drawButtons(int selectedIdx) {
  ...
  // 🔥 FIX: Wis HELE knoppenbalk EERST (inclusief highlight ruimte)
  gfx->fillRect(startX - 4, y - 4, totalW + 8, h + 8, 0x0000);
  ...
  gfx->setFont(&FreeSansOblique9pt7b);  // FreeFont voor button labels
  ...
  for (int i = 0; i < 5; i++) {
    ...
    gfx->setCursor(x + (w - tw) / 2 - x1, y + (h / 2) + 5);  // 3px omhoog

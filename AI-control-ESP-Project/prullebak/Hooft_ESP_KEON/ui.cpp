...
static void drawConnectionPopup(){
  ...
  int bx = R_WIN_X + 20, by = R_WIN_Y + 80, bw = R_WIN_W - 40, bh = 80;
  ...
  if (isConnected) gfx->print("Verbreken met ");
  else gfx->print("Verbinden met ");
  gfx->print(deviceName);
  gfx->print("?");
  gfx->setFont(nullptr); gfx->setTextSize(1);
  gfx->setTextColor(0x8410, CFG.COL_BG);
  gfx->setCursor(bx + 10, by + bh - 12);
  gfx->print("Z: OK   C: annuleer");
}
//static void drawConnectionPopup(){
  //if (!connectionPopupOpen) return;
...
// SIMPELE CONNECTION POPUP
  if (connectionPopupOpen) {
    if (zEdge) {
      if (connectionDeviceIdx == 0) {
        if (keonConnected) disconnectKeon();
        else { Serial.println("[KEON] Connecting..."); keonConnected = keonConnect(); Serial.printf("[KEON] %s\n", keonConnected ? "OK" : "FAIL"); }
      } else if (connectionDeviceIdx == 1) {
        if (solaceConnected) solaceConnected = false;
        else solaceConnected = (random(100) < 75);
      }
...
//
  if (nk//Ready) {
    bool o//k = nchuk.update();
    if (//!ok) nkReady = nchuk.connect();
    jx = n//chuk.joyX(); jy = nchuk.joyY();
    cNow =// nchuk.buttonC(); zNow = nchuk.buttonZ();
    //
    // Deb//ug nunchuk button status (alleen als er een knop ingedrukt is)
    stat//ic bool lastZNow = false;
    if// (zNow != lastZNow) {
      Se//rial.printf("[NUNCHUK] Z button: %s\n", zNow ? "PRESSED" : "RELEASED");
      la//stZNow = zNow;
    }//
  } el//se {
    st//atic uint32_t lastTry=0;

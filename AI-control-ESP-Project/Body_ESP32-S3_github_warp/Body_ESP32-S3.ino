...
// ===== CALIBRATIE HELPER =====
//static bool calibrationMode = false;  // ← Zet op TRUE om te activeren
//static bool calibrationMode = true;  // ← GEACTIVEERD!
static bool lastCalTouchState = false;
static int16_t lastCalTouchX = -1, lastCalTouchY = -1;

// ===== CALIBRATIE HELPER FUNCTIE =====
void drawCalibrationCrosshair(int16_t x, int16_t y) {
  // Teken crosshair op touch positie
  body_gfx->drawFastVLine(x, 0, 240, 0xFFFF);      // Witte verticale lijn
  body_gfx->drawFastHLine(0, y, 320, 0xFFFF);      // Witte horizontale lijn
  body_gfx->fillCircle(x, y, 5, 0xF800);           // Grote rode dot
  body_gfx->drawCircle(x, y, 10, 0xF800);          // Rode ring
  
  // Toon coordinaten groot en leesbaar
  body_gfx->fillRect(0, 0, 120, 30, 0x0000);
  body_gfx->drawRect(0, 0, 120, 30, 0xFFFF);
  body_gfx->setTextColor(0xFFFF, 0x0000);
  body_gfx->setTextSize(2);
  body_gfx->setCursor(5, 8);
  body_gfx->printf("X:%3d Y:%3d", x, y);
  
  Serial.printf("[CAL] Touch at X=%d, Y=%d\n", x, y);
}

...
// Button rectangles - EXACT zoals getekend
...
void menu_begin(TFT_eSPI* gfx){
  ...
  // Touch zones EXACT gelijk aan visuele buttons - GEEN OFFSETS!
  bxOverruleX = btnX; 
  bxOverruleY = startY; 
  bxOverruleW = btnW; 
  bxOverruleH = btnH;
  ...
  Serial.printf("[MENU] Button zones: AI(%d,%d,%d,%d) ML(%d,%d,%d,%d) SENSORS(%d,%d,%d,%d)\n",
                bxOverruleX, bxOverruleY, bxOverruleW, bxOverruleH,
                bxMLTrainingX, bxMLTrainingY, bxMLTrainingW, bxMLTrainingH,
                bxSensorsX, bxSensorsY, bxSensorsW, bxSensorsH);
}
...
MenuEvent menu_poll(){
  ...
  if (wasTouching && !isTouching) {
    ...
    // TEMP: Menu-specifieke Y-correctie
    // Vaste offset voor bovenste helft van scherm
    if (y < 100) {
    ...
    // X-correctie: non-lineair patroon zoals hoofdscherm
    // Rechts heeft meer offset dan links
    if (x > 200) {
        int16_t overshoot = x - 200;
        x -= (overshoot * 15) / 100;  // Progressieve correctie rechts
    }
    
    // DEBUG: Print touch coordinaten (NA correctie)

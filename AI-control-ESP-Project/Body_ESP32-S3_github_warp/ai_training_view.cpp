...
static void drawNavButton(int16_t x,int16_t y,int16_t w,int16_t h,const char* txt, uint16_t color, bool selected = false) {
  ...
  g->setTextSize(1);
  // TFT_eSPI text centering schatting voor size 1
  uint16_t tw = strlen(txt) * 6;  // 6 pixels per karakter voor size 1
  uint16_t th = 8;                // 8 pixels hoog voor size 1
  int textX = x + (w - tw)/2;
...
static bool loadNextDataPoint() {
  ...
  fs::File file = SD_MMC.open("/" + currentFile);
  //File file = SD_MMC.open("/" + currentFile);
...
void aiTraining_begin(TFT_eSPI* gfx, const String& filename) {
  g = gfx;
  SCR_W = g->width();
  SCR_H = g->height();
...
TrainingResult aiTraining_poll() {
  ...
  // TFT_eSPI text centering schatting voor size 1
  uint16_t w1 = progressText.length() * 6;  // 6 pixels per karakter voor size 1
  ...
  if (trainingComplete) {
    ...
    // TFT_eSPI text centering schatting voor size 2
    uint16_t tw = completeText.length() * 12;  // 12 pixels per karakter voor size 2
    ...
    // TFT_eSPI text centering schatting voor size 1
    tw = infoText.length() * 6;  // 6 pixels per karakter voor size 1
    g->setCursor((SCR_W - tw) / 2, 80);
    g->print(infoText);
    
    String saveText = "Kies OPSLAAN om model te bewaren";
    // TFT_eSPI text centering schatting voor size 1
    tw = saveText.length() * 6;  // 6 pixels per karakter voor size 1
  ...
  if (aiQuestion.length() > 38) {  // Korter maken voor betere verdeling
    ...
    if (splitPos > 15) {
      ...
      // Teken regel 1
      // TFT_eSPI text centering schatting voor size 1
      uint16_t w1 = line1.length() * 6;  // 6 pixels per karakter voor size 1
      g->setCursor((SCR_W - w1) / 2, 32);
      g->print(line1);
      
      // Teken regel 2
      uint16_t w2 = line2.length() * 6;  // 6 pixels per karakter voor size 1
      g->setCursor((SCR_W - w2) / 2, 48);
      g->print(line2);
    } else {
      // Kan niet splitsen, gebruik 1 regel
      // TFT_eSPI text centering schatting voor size 1
      uint16_t w2 = aiQuestion.length() * 6;  // 6 pixels per karakter voor size 1
  ...
  g->setTextSize(1);
  // TFT_eSPI text centering schatting voor size 1
  uint16_t fw = fullAnswer.length() * 6;  // 6 pixels per karakter voor size 1
  ...
  // TFT_eSPI text centering schatting voor size 1
  uint16_t pw = answerPrefix.length() * 6;  // 6 pixels per karakter voor size 1
  ...
  // TFT_eSPI text centering schatting voor size 0
  uint16_t fw1 = fullLine1.length() * 5;  // 5 pixels per karakter voor size 0
  ...
  g->print(hartText);
  // TFT_eSPI text centering schatting voor size 0
  uint16_t tw1 = hartText.length() * 5;  // 5 pixels per karakter voor size 0
  currentX1 += tw1;
  ...
  // TFT_eSPI text centering schatting voor size 0
  uint16_t fw2 = fullLine2.length() * 5;  // 5 pixels per karakter voor size 0
  ...
  g->print(tempText);
  // TFT_eSPI text centering schatting voor size 0
  uint16_t tw2 = tempText.length() * 5;  // 5 pixels per karakter voor size 0
  currentX2 += tw2;
  ...
  g->print(ademText);
  // TFT_eSPI text centering schatting voor size 0
  tw2 = ademText.length() * 5;  // 5 pixels per karakter voor size 0
  currentX2 += tw2;
  ...
  // TFT_eSPI text centering schatting voor size 0
  uint16_t fw3 = fullLine3.length() * 5;  // 5 pixels per karakter voor size 0
  ...
  g->print(snelhText);
  // TFT_eSPI text centering schatting voor size 0
  uint16_t tw3 = snelhText.length() * 5;  // 5 pixels per karakter voor size 0
  currentX3 += tw3;
  ...
  g->print(zuigenText);
  // TFT_eSPI text centering schatting voor size 0
  tw3 = zuigenText.length() * 5;  // 5 pixels per karakter voor size 0
  currentX3 += tw3;

...
void inputTouchBegin(TFT_eSPI* gfx){
  ...
  bh = BUTTON_H;
  by = BUTTON_Y;
  bx1 = BUTTON_START_X + 0 * BUTTON_SPACING;
  bx2 = BUTTON_START_X + 1 * BUTTON_SPACING;
  bx3 = BUTTON_START_X + 2 * BUTTON_SPACING;
  bx4 = BUTTON_START_X + 3 * BUTTON_SPACING;

#if ENABLE_TOUCH
  SPI.begin(TOUCHSCREEN_SCLK_PIN, TOUCHSCREEN_MISO_PIN, TOUCHSCREEN_MOSI_PIN);
  
  // EXACTE volgorde van T-HMI voorbeeld:
  touch.begin(240, 320);  // ← Portrait dimensies zoals voorbeeld
  touch.setCal(275, 1788, 325, 1860, 240, 320);   // v20a
  touch.setRotation(3);  // ← Rotation NA setCal
  
  Serial.println("[TOUCH] T-HMI factory baseline - correct order");
...
//#if ENABLE_TOUCH
  ...
  // NEGEER EEPROM - gebruik ALLEEN handmatige waarden
  //Serial.println("[TOUCH] EEPROM BYPASSED - using manual calibration");
  ...
  //} else {
    ...
    //touch.setCal(285, 1788, 311, 1877, 240, 320);   // Terug naar factory dimensies
    //touch.setCal(311, 1877, 285, 1788, 320, 240);   // v5
    //touch.setCal(385, 1788, 341, 1877, 240, 320);   // v6
    //touch.setCal(285, 1580, 365, 1820, 240, 320);   // v7
    //touch.setCal(320, 1520, 395, 1780, 240, 320);   // v8
    //touch.setCal(320, 1420, 417, 1780, 240, 320);   // v9
    //touch.setCal(285, 1650, 365, 1820, 240, 320);   // v10
    //touch.setCal(285, 1650, 369, 1820, 240, 320);   // V11
    //touch.setCal(285, 1650, 369, 1820, 320, 240);   // v12
    //touch.setCal(311, 1788, 285, 1877, 320, 240);  // v14
    ...
    //Serial.println("[TOUCH] Clean slate test - factory baseline");
  //Serial.printf("[TOUCH] Screen: %dx%d, Touch zones: Y=%d\n", SCR_W, SCR_H, by);
  //}

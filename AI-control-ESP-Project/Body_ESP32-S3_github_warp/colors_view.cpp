...
// TFT_eSPI wordt via body_gfx4.h geïncludeerd
...
static void drawButton(int16_t x,int16_t y,int16_t w,int16_t h,const char* txt, uint16_t color) {
  ...
  // TFT_eSPI text centering schatting
...
void colors_begin(TFT_eSPI* gfx) {
    g = gfx;
    SCR_W = g->width();
    SCR_H = g->height();
    ...
    uint16_t tw = strlen("Kleuren") * 12;  // 12 pixels per karakter voor size 2
...
ColorEvent colors_poll() {
    ...
    if (isTouching) {
        ...
        lastY = y;
        wasTouching = true;
        return CE_NONE;  // Nog niet triggeren tijdens aanraken
    }
    ...
    if (wasTouching && !isTouching) {
        ...
        Serial.printf("[COLORS] Touch: (%d,%d)\n", x, y);

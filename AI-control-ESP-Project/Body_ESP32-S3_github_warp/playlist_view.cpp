...
#include <SD_MMC.h>
#include <FS.h>  // FIX: Toegevoegd voor File type
...
// FIX: BLACK en WHITE vervangen door TFT_BLACK en TFT_WHITE
...
static void scanSDFiles() {
  ...
  // FIX: Gebruik fs::File voor expliciet type
...
static void drawButton(int16_t x,int16_t y,int16_t w,int16_t h,const char* txt, uint16_t color) {
  ...
  // FIX: BLACK vervangen door TFT_BLACK
...
static void drawFileList() {
  ...
  for (int i = 0; i < fileCount; i++) {
    ...
    // FIXED: Gebruik SD_MMC voor file size check met fs::File
...
PlaylistEvent playlist_poll() {
  ...
  Serial.printf("[PLAYLIST] Touch: (%d,%d)\n", x, y);

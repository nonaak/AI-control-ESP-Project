...
// ═══════════════════════════════════════════════════════════
// ORGASM & COOLDOWN STATE MACHINE
// ═══════════════════════════════════════════════════════════

enum OrgasmState {
  ORGASM_NONE = 0,        // Normale sessie
  ORGASM_TRIGGERED = 1,   // Y-hold 3 sec gedetecteerd → MAX speed
  ORGASM_ACTIVE = 2,      // Wacht op C-button ("ik ben klaar!")
  COOLDOWN_ACTIVE = 3     // Level 0, cooldown timer/counter loopt
};

static OrgasmState orgasmState = ORGASM_NONE;
static uint32_t orgasmTriggerTime = 0;
static uint32_t cooldownStartTime = 0;
static uint32_t cooldownStartStrokes = 0;
static uint32_t cooldownSeconds = 0;  // Wordt gevuld vanuit CFG of Body ESP
static uint32_t cooldownStrokes = 0;
...
// ═══════════════════════════════════════════════════════════
// ORGASM BACKGROUND FLASH
// ═══════════════════════════════════════════════════════════
static uint16_t getOrgasmBackgroundColor() {
  // Normale achtergrond als niet in orgasm mode
  if (orgasmState != ORGASM_TRIGGERED && orgasmState != ORGASM_ACTIVE) {
    return CFG.COL_BG;
  }
  
  // Knipper tussen rood en zwart (300ms interval)
...
//CEvent cev = pollCEvent(cNow);
  ...
  // ═══════════════════════════════════════════════════════════
  // Y-AXIS HOLD DETECTION (3 sec) - ORGASM TRIGGER
  // ═══════════════════════════════════════════════════════════

  static uint32_t yHoldStart = 0;
  static bool yHoldTriggered = false;

  if (jy > JY_HI && !paused && uiMode == MODE_ANIM && orgasmState == ORGASM_NONE) {  
    // Y omhoog, niet gepauzeerd, alleen in normale sessie
    if (yHoldStart == 0) {
      yHoldStart = millis();
      Serial.printf("[NUNCHUK] Y hold started! jy=%d > JY_HI=%d\n", jy, JY_HI);
    }
  
    uint32_t holdTime = millis() - yHoldStart;
  
    // Debug elke seconde
    static uint32_t lastHoldDebug = 0;
    if (millis() - lastHoldDebug > 1000) {
      Serial.printf("[Y-HOLD] Holding... %lu ms / 3000 ms\n", holdTime);
      lastHoldDebug = millis();
    }
  
    // Trigger na 3 seconden
    if (holdTime >= 3000 && !yHoldTriggered) {
      // ═══ ORGASM TRIGGER! ═══
      orgasmState = ORGASM_TRIGGERED;
      orgasmTriggerTime = millis();
      g_speedStep = CFG.SPEED_STEPS - 1;  // MAX speed (7)
      yHoldTriggered = true;
    
      // Stuur naar Body ESP
      extern void sendOrgasmTrigger();
      sendOrgasmTrigger();
    
      Serial.println("[ORGASM] ✅ TRIGGERED! Speed → MAX. Press C when done.");
    
     // State transitions happen automatically:
    // TRIGGERED → ACTIVE (direct)
      orgasmState = ORGASM_ACTIVE;
    }
  } else {
    // Reset als Y niet meer omhoog
   if (yHoldStart != 0) {
    Serial.printf("[NUNCHUK] Y hold released after %lu ms\n", millis() - yHoldStart);
    }
    yHoldStart = 0;
  ...
  // ═══════════════════════════════════════════════════════════
  // NUNCHUK JOYSTICK HOLD DETECTION
  // ═══════════════════════════════════════════════════════════
  // Y-axis hold detection (4 seconds) - ORGASM TRIGGER
  //static uint32_t yHoldStart = 0;
  //static bool yHoldTriggered = false;
  
  //if (jy > JY_HI && !paused && uiMode == MODE_ANIM) {  // ← GEBRUIK JY_HI!
    //if (yHoldStart == 0) {
      //yHoldStart = millis();
      //Serial.printf("[NUNCHUK] Y hold started! jy=%d > JY_HI=%d\n", jy, JY_HI);
    //}
    ...
    // Trigger na 3 seconden
    //if (holdTime >= 3000 && !yHoldTriggered) {
      //Serial.println("[ORGASM] ✅ Y held 3 sec - Triggering orgasm!");
      //extern void sendOrgasmTrigger();
      //sendOrgasmTrigger();
      //yHoldTriggered = true;
    //}
  ...
  // X-axis hold detection (2 seconds) - FUNSCRIPT TOGGLE
  static uint32_t xHoldStart = 0;
  static int8_t xHoldDir = 0;  // -1=links, +1=rechts, 0=center
  static bool xHoldTriggered = false;
  
  if (jx < JX_LO && xHoldDir != -1 && uiMode == MODE_ANIM) {  // X links
    xHoldStart = millis();
    xHoldDir = -1;
    xHoldTriggered = false;
    Serial.println("[NUNCHUK] X hold LEFT started...");
  } else if (jx > JX_HI && xHoldDir != 1 && uiMode == MODE_ANIM) {  // X rechts
    xHoldStart = millis();
    xHoldDir = 1;
    xHoldTriggered = false;
    Serial.println("[NUNCHUK] X hold RIGHT started...");
  } else if (jx >= JX_LO && jx <= JX_HI) {  // X center
    // Check of hold lang genoeg was
    if (xHoldDir != 0 && !xHoldTriggered) {
      uint32_t holdTime = millis() - xHoldStart;
      
      if (holdTime >= 2000) {
        // Trigger funscript command
        extern void sendFunscriptCommand(bool enabled);
        
        if (xHoldDir == -1) {
          Serial.println("[FUNSCRIPT] ✅ X LEFT 2 sec - Funscript OFF");
          sendFunscriptCommand(false);
        } else {
          Serial.println("[FUNSCRIPT] ✅ X RIGHT 2 sec - Funscript ON");
          sendFunscriptCommand(true);
        }
        xHoldTriggered = true;
      } else {
        Serial.printf("[NUNCHUK] X hold released after %lu ms (too short)\n", holdTime);
      }
    }
    xHoldDir = 0;
    xHoldStart = 0;
...
if (nunchukHelpPopupOpen) {
  ...
  // ← VERWIJDER DE LOSSE } HIER!
  
  if (paletteOpen && cev==CE_SHORT){  // ← MOET 'if' zijn!
    ...
    // ═══════════════════════════════════════════════════════════
    // C-BUTTON: ORGASM COMPLETE OF PAUSE/UNPAUSE
    // ═══════════════════════════════════════════════════════════
    
    if (orgasmState == ORGASM_ACTIVE) {
      // ═══ ORGASM COMPLETE → START COOLDOWN ═══
      Serial.println("[ORGASM] C pressed - User signaled orgasm complete!");
      
      orgasmState = COOLDOWN_ACTIVE;
      cooldownStartTime = millis();
      
      // Haal punch count op (extern in animatie code)
      extern uint32_t punchCount;
      cooldownStartStrokes = punchCount;
      
      // Laad cooldown settings uit config
      cooldownSeconds = CFG.cooldownDefaultSeconds;
      cooldownStrokes = CFG.cooldownDefaultStrokes;
      
      // Auto naar level 0 als enabled
      if (CFG.orgasmAutoLevel0) {
        g_speedStep = 0;
        Serial.println("[COOLDOWN] Auto level 0 enabled");
      }
      
      // Stuur naar Body ESP
      machineStatus_message_t cooldownMsg;
      memset(&cooldownMsg, 0, sizeof(cooldownMsg));
      strcpy(cooldownMsg.command, "ORGASM_COMPLETE");
      extern void sendBodyESPStatusUpdate(const machineStatus_message_t &msg);
      sendBodyESPStatusUpdate(cooldownMsg);
      
      Serial.printf("[COOLDOWN] Started: %u sec OR %u strokes (mode %u)\n", 
                    cooldownSeconds, cooldownStrokes, CFG.cooldownMode);
      
    } else {
      // ═══ NORMALE PAUSE/UNPAUSE ═══
      if (!paused) {
        parkToBottom = true;
        paused = true;
...
// Use Keon frequencies for animation sync!
  ...
  float topPercent = CFG.strokeTopLimit / 100.0f;
  float bottomPercent = CFG.strokeBottomLimit / 100.0f;
  int CAP_Y_OUT = (int)round(CAP_Y_OUT_BASE + (CAP_Y_IN_BASE - CAP_Y_OUT_BASE) * topPercent);
  int CAP_Y_IN  = (int)round(CAP_Y_OUT_BASE + (CAP_Y_IN_BASE - CAP_Y_OUT_BASE) * bottomPercent);
  //const float RANGE_SCALE  = 0.90f;
  //int CAP_Y_IN  = (int)round(CAP_Y_MID + (CAP_Y_IN_BASE  - CAP_Y_MID) * RANGE_SCALE);
  //int CAP_Y_OUT = (int)round(CAP_Y_MID + (CAP_Y_OUT_BASE - CAP_Y_MID) * RANGE_SCALE);
  ...
  if (uiMode == MODE_ANIM) {
    uint16_t bgColor = getOrgasmBackgroundColor();  // ✅ NIEUW
    cv->fillScreen(bgColor);                        // ✅ NIEUW
  ...
  // ═══════════════════════════════════════════════════════════
  // COOLDOWN CHECK - Check of cooldown compleet is
  // ═══════════════════════════════════════════════════════════

  if (orgasmState == COOLDOWN_ACTIVE) {
    extern uint32_t punchCount;

    bool timeComplete = false;
    bool strokesComplete = false;
    
    // Check tijd
    if (CFG.cooldownMode == 0 || CFG.cooldownMode == 2) {
      timeComplete = (millis() - cooldownStartTime) >= (cooldownSeconds * 1000);
    }

    // Check strokes
    if (CFG.cooldownMode == 1 || CFG.cooldownMode == 2) {
      strokesComplete = (punchCount - cooldownStartStrokes) >= cooldownStrokes;
  
      // ✅ DEBUG ELKE SECONDE!
      static uint32_t lastStrokeCheck = 0;
      if (millis() - lastStrokeCheck > 1000) {
        Serial.printf("[STROKE-DBG] NOW=%u START=%u DIFF=%u TARGET=%u DONE=%d\n",
                      punchCount, cooldownStartStrokes, 
                      (punchCount - cooldownStartStrokes), cooldownStrokes, strokesComplete);
        lastStrokeCheck = millis();
      }
    }
    ...
    // Bepaal of cooldown klaar is
    bool cooldownDone = false;
    if (CFG.cooldownMode == 0) {
      cooldownDone = timeComplete;
    } else if (CFG.cooldownMode == 1) {
      cooldownDone = strokesComplete;
    } else if (CFG.cooldownMode == 2) {
      cooldownDone = (timeComplete || strokesComplete);  // Eerste wint
    }
    
    // Cooldown compleet!
    if (cooldownDone) {
      orgasmState = ORGASM_NONE;
      ...
      // Stuur naar Body ESP
      machineStatus_message_t resumeMsg;
      memset(&resumeMsg, 0, sizeof(resumeMsg));
      strcpy(resumeMsg.command, "COOLDOWN_COMPLETE");
      extern void sendBodyESPStatusUpdate(const machineStatus_message_t &msg);
      sendBodyESPStatusUpdate(resumeMsg);
      
      Serial.println("[COOLDOWN] ✅ Complete! → PAUSED");
    }
    
    // Debug elke 5 seconden
    static uint32_t lastCooldownDebug = 0;
    if (millis() - lastCooldownDebug > 5000) {
      //extern uint32_t punchCount;
      uint32_t elapsedSec = (millis() - cooldownStartTime) / 1000;
      uint32_t elapsedStrokes = punchCount - cooldownStartStrokes;
      Serial.printf("[COOLDOWN] Progress: %u/%u sec, %u/%u strokes\n",
                    elapsedSec, cooldownSeconds, elapsedStrokes, cooldownStrokes);
